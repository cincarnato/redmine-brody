...
module AgileVersionsHelper
  ...
  def column_version_class(version)
    version ? ".version-#{version.id}-issues" : '.no-version-issues'
  end

  def agile_version_query_links(title, queries)
    return '' if queries.empty?
    # links to #index on issues/show
    url_params = { controller: 'agile_versions', action: 'index', project_id: @project}
  ...
  def render_sidebar_agile_version_queries
    ...
    out << agile_version_query_links(l(:label_agile_board_plural), sidebar_agile_version_queries.select(&:is_public?))
    out
  end
end
